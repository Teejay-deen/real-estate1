...
const Search = () => {
  ...
  return (
    <div className="px-[20px] max-w-[1170px] py-6 mx-auto flex flex-col lg:flex-row justify-between gap-4 lg:gap-x-4 relative lg:top-4 lg:shadow-1 bg-white lg:bg-transparent lg:backdrop-blur rounded-full">
      ...
      <PropertyDropDown />
      <PriceRangeDropDown />
      <button onClick={()=> handleClick()} className="bg-violet-700 hover:bg-violet-800 text-white transition w-full lg:max-w-[162px] h-16 rounded-full flex justify-center items-center text-lg">
        <RiSearch2Line />
      </button>
    </div>
  );
